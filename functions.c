...
void edit_task (int task_id){
    ...
    char changes[1000] = ""; //to store the changes made
    ...
    {
        int subtask_id;
        printf("Enter the subtask id to edit: ");
        scanf(" %d", &subtask_id);
        edit_subtask(&temp_task, subtask_id);
        printf("Do you want to edit another subtask? (Y/N): ");
        scanf(" %c", &choice);
...
void delete_task(int task_id){
    ...
    //deleting all subtasks associated with the task
    subtask *current_sub = task_ptr->subtask_ptr;
    while (current_sub != NULL) {
        subtask *temp = current_sub;
        current_sub = current_sub->next_subtask_ptr;
        free(temp);
    }
    free(task_ptr);
...
void display_some_task_details(task* task_ptr){
    char colour[14];
    if (task_ptr->category_id > 0) {
        strcpy(colour, category[task_ptr->category_id - 1].colour); 
    } 
    else {
        strcpy(colour, "");
    }
...
void get_current_recurrence_number(task *ptr){
    ...
    {
        if (ptr->interval <= 0) return; // or break to avoid infinite loop
        occurrence_no++;
...
void edit_subtask (task *parent_task, int subtask_id){
    if (parent_task == NULL)
    {
        printf("Parent task with id %d not found.\n", parent_task->task_id);
        return;
    ...
    subtask *subtask_ptr = find_subtask_by_id(parent_task, subtask_id);
    subtask temp_subtask = *subtask_ptr; // Create a copy of the original subtask
    if (subtask_ptr != NULL)
    {
        printf("Editing subtask with id %d of task with id %d:\n", subtask_id, parent_task->task_id);
        char choice;
        printf("Do you want to edit subtask name? (Y/N): ");
        scanf(" %c", &choice);
        if (choice == 'Y' || choice == 'y')
        {
            printf("Enter new subtask name (current: %s): ", temp_subtask.subtask_name);
            scanf(" %99[^\n]", temp_subtask.subtask_name);
        }
        ...
        if (choice == 'Y' || choice == 'y')
        {
            printf("Enter new subtask description (current: %s): ", temp_subtask.subtask_description);
            scanf(" %999[^\n]", temp_subtask.subtask_description);
        }
        printf("Do you want to confirm the changes to subtask with id %d of task with id %d?(Y/N): ", subtask_id, parent_task->task_id);
        scanf(" %c", &choice);
        if (choice != 'Y' && choice != 'y')
        {
            printf("Changes discarded. Subtask with id %d of task with id %d remains unchanged.\n", subtask_id, parent_task->task_id);
            return;
        }
        //else
        *subtask_ptr = temp_subtask; // Apply the changes
...
void display_categories(){
    ...
    for (int i = 0; i < category_amount; i++)
    {
        ...
        if (strcmp(category[i].colour, colour_red)==0)
            printf("Category colour: Red\n");
        else if (strcmp(category[i].colour, colour_orange)==0)
            printf("Category colour: Orange\n");
        else if (strcmp(category[i].colour, colour_yellow)==0)
            printf("Category colour: Yellow\n");
        else if (strcmp(category[i].colour, colour_green)==0)
            printf("Category colour: Green\n");
        else if (strcmp(category[i].colour, colour_blue)==0)
            printf("Category colour: Blue\n");
        else if (strcmp(category[i].colour, colour_cyan)==0)
            printf("Category colour: Cyan\n");
        else if (strcmp(category[i].colour, colour_magenta)==0)
            printf("Category colour: Magenta\n");
        else if (strcmp(category[i].colour, colour_brown)==0)
            printf("Category colour: Brown\n");
        else
            printf("Category colour: None\n");
    }
}
void edit_category(){
    int id;
...
void delete_category(){
    ...
    for (int i = id-1; i < category_amount-1; i++)
    {
        category[i] = category[i+1];
        category[i].category_id = i+1;
    }
    category_amount--;
    printf("Category with id %d deleted successfully.\n", id);
}
...
void auto_sort_f(){
    ...
    while (ptr != NULL)
    {
        //changing subtask parent ids
        subtask *sub_ptr = ptr->subtask_ptr;
        while (sub_ptr != NULL)
        {
            sub_ptr->parent_task_id = id;
...
void load_from_file()
{
    ...
    else
    {
        ...
        //updating next_available_id
        if (tail_ptr != NULL)
        {
            next_available_id = tail_ptr->task_id + 1;
        }
        fclose(task_file);
    }
    ...
    else
    {
        ...
        fread(&category_amount, sizeof(int), 1, category_file);
...
void save_to_file(){
    ...
    else
    {
        ...
        fwrite(&category_amount, sizeof(int), 1, category_file);
